          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('userProfileName')}>
                  User Profile Name <FontAwesomeIcon icon={getSortIconByFieldName('userProfileName')} />
                </th>
                <th className="hand" onClick={sort('email')}>
                  Email <FontAwesomeIcon icon={getSortIconByFieldName('email')} />
                </th>
                <th className="hand" onClick={sort('isReportSharingEnabled')}>
                  Is Report Sharing Enabled <FontAwesomeIcon icon={getSortIconByFieldName('isReportSharingEnabled')} />
                </th>
                <th className="hand" onClick={sort('isActive')}>
                  Is Active <FontAwesomeIcon icon={getSortIconByFieldName('isActive')} />
                </th>
                <th>
                  User Account <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {userProfileList.map((userProfile, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{userProfile.userProfileName}</td>
                  <td>{userProfile.email}</td>
                  <td>{userProfile.isReportSharingEnabled}</td>
                  <td>{userProfile.isActive}</td>
                  <td>
                    {userProfile.userAccount ? (
                      <Link to={`/user-account/${userProfile.userAccount.id}`}>{userProfile.userAccount.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
